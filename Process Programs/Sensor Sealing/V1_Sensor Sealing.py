...
# ----- Label 2 Position Adjustments (QR Code & Text) -----
# QR Code positioning
QR_SIZE = 225        # Size of QR code in pixels
QR_X_OFFSET = 50     # Distance from right edge (increase to move left, decrease to move right)
QR_Y_OFFSET = -20   # Vertical position (negative moves up, positive moves down)

# Sensor text positioning
TEXT_X = 60          # Horizontal position (increase to move right, decrease to move left)
TEXT_Y = 30          # Vertical position (increase to move down, decrease to move up)

# ----- Printer Settings -----
PRINT_HEAT = 25      # Heat/Darkness (0-20, higher = darker) - increase if print is faded
PRINT_SPEED = 5      # Print speed (2-5, lower = slower but better quality)
...
def get_product_description(sensor_id):
    """Fetch product description from lot_masterlist based on sensor_id"""
    default = ""  # Return empty string if not found
    try:
        conn = sqlite3.connect(db_path_masterlist)
        cur = conn.cursor()
        # Try different possible column names for product description
        for colname in ('"Product_Description"', 'Product_Description', 'product_description', 'ProductDescription', 'Description'):
            try:
                cur.execute(f"SELECT {colname} FROM lot_masterlist WHERE sensor_id = ? LIMIT 1", (sensor_id,))
                res = cur.fetchone()
...
# ----- Helper: get allowed sensor ids for current process (no defects from previous processes)
...
# Generate Label 1: Product Description (from database based on sensor_id)
def make_label1_image(sensor_id):
    """Render product description on Label 1 as two lines.

    Splits the product description at the final dash so the last segment
    appears on the second line (e.g. 'MAGS-PS01-022601' -> 'MAGS-PS01-' / '022601').
    The font is auto-scaled to best fit the printable area.
    """
    label1 = Image.new("RGB", (WIDTH_PX, HEIGHT_PX), "white")
    draw = ImageDraw.Draw(label1)

    # Get product description from database; fallback to sensor_id if not found
    prod_desc = get_product_description(sensor_id) or (sensor_id or "")
    prod_desc = prod_desc.strip()

    # Prepare two-line text split at the last dash
    parts = prod_desc.split('-') if prod_desc else []
    if len(parts) <= 1:
        lines = [prod_desc]
    else:
        first = "-".join(parts[:-1])
    ...
    # Fit font size to available area
    margin = 3
    max_width = WIDTH_PX - margin * 2
    max_height = HEIGHT_PX - margin * 2
    chosen_font = None
    chosen_sizes = (0, 0, 0, 0)

    for fs in range(48, 6, -1):
        try:
            f = ImageFont.truetype("arialbd.ttf", fs)
        except Exception:
            f = ImageFont.load_default()

        try:
            b0 = draw.textbbox((0, 0), lines[0], font=f)
            w0 = b0[2] - b0[0]
            h0 = b0[3] - b0[1]
            if len(lines) > 1 and lines[1]:
                b1 = draw.textbbox((0, 0), lines[1], font=f)
                w1 = b1[2] - b1[0]
                h1 = b1[3] - b1[1]
            else:
                w1 = 0
        ...
        total_h = h0 + (h1 if h1 else 0) + (2 if h1 else 0)
        if max(w0, w1) <= max_width and total_h <= max_height:
            chosen_font = f
            chosen_sizes = (w0, h0, w1, h1)
            break

    if chosen_font is None:
        try:
            chosen_font = ImageFont.truetype("arialbd.ttf", 12)
        except Exception:
            chosen_font = ImageFont.load_default()
        b0 = draw.textbbox((0, 0), lines[0], font=chosen_font)
        w0 = b0[2] - b0[0]
        h0 = b0[3] - b0[1]
        if len(lines) > 1 and lines[1]:
            b1 = draw.textbbox((0, 0), lines[1], font=chosen_font)
            w1 = b1[2] - b1[0]
            h1 = b1[3] - b1[1]
        else:
            w1 = 0
    ...
    w0, h0, w1, h1 = chosen_sizes
    total_h = h0 + (h1 if h1 else 0) + (2 if h1 else 0)
    start_y = (HEIGHT_PX - total_h) // 2

    # Draw first line centered
    x0 = (WIDTH_PX - w0) // 2
    draw.text((x0, start_y), lines[0], font=chosen_font, fill="black")

    # Draw second line if present
    if len(lines) > 1 and lines[1]:
        x1 = (WIDTH_PX - w1) // 2
        draw.text((x1, start_y + h0 + 2), lines[1], font=chosen_font, fill="black")
...
def make_label2_image(sensor_text):
    ...
    qr_x = WIDTH_PX - QR_SIZE - QR_X_OFFSET
    qr_y = QR_Y_OFFSET
    ...
    if len(parts) >= 4:
        # Merge first two parts into one line
...
def generate_ezpl_label(label_image, label_num):
    ...
    commands.append(f"^H{PRINT_HEAT}")     # Set heat/darkness (adjustable via PRINT_HEAT variable)
    commands.append(f"^S{PRINT_SPEED}")    # Set print speed (adjustable via PRINT_SPEED variable)
...
# ----- Helper: update print status in database -----
def update_sensor_print_status(lot_number, sensor_id, current_process, status, operator='', quantity_in=None, quantity_out=None, remarks=''):
    """Update `lot_tracking` with print status for a single sensor.
    Uses `process_column_mapping` to map fields (if available).
    Returns True if update succeeded, False otherwise.
    """
    try:
        cols = process_column_mapping.get(current_process)
        if not cols:
            return False
        ...
        # Expected ordering in mapping: qty_in, qty_out, status, remarks, datetime, operator
        if len(cols) >= 1 and quantity_in is not None:
            set_parts.append(f"{cols[0]}=?")
            params.append(quantity_in)
        if len(cols) >= 2 and quantity_out is not None:
            set_parts.append(f"{cols[1]}=?")
            params.append(quantity_out)
        if len(cols) >= 3:
            set_parts.append(f"{cols[2]}=?")
            params.append(status)
        if len(cols) >= 4:
            set_parts.append(f"{cols[3]}=?")
            params.append(remarks)
        if len(cols) >= 5:
            set_parts.append(f"{cols[4]}=?")
            params.append(datetime.now().strftime("%Y-%m-%d %I:%M:%S %p"))
        if len(cols) >= 6:
            set_parts.append(f"{cols[5]}=?")
            params.append(operator)

        if not set_parts:
            return False

        params.extend([lot_number, sensor_id])

        sql = f"UPDATE lot_tracking SET {', '.join(set_parts)} WHERE lot_number=? AND sensor_id=?"
        ...
        print(f"update_sensor_print_status error: {e}")
        return False
...
# ----- UI: main window -----
root = tk.Tk()
root.title("Sensor Sealing")
root.geometry("1000x660")  # Adjusted width
root.configure(bg="lightblue")
root.resizable(False, False)

# Title
title_label = tk.Label(root, text="Sensor Sealing", font=("BiomeW04-Bold", 16, "bold"), bg="lightblue")
...
def fetch_lot_info(event=None):
    ...
    try:
        ...
        if str(lot_condition).upper() == "MP":
            if current_process != "Sensor Sealing":  # Exact match for comparison
                error_msg = f"The lot number inputted is not for Sensor Sealing.\nCurrent process: {current_process}"
                if conn_track:
                    conn_track.close()
                if conn_master:
                    conn_master.close()
        ...
        process_to_columns = {
            ...
            "Sensor Sealing": []  # Sensor Sealing doesn't store measurement values in masterlist
        }
        ...
        conn_track.close()
        # Unlock first sensor row so user can scan or type the first sensor ID
        try:
            if sensor_id_labels:
                # Ensure all rows are disabled first, then enable the first
                for i in range(len(sensor_id_labels)):
                    try:
                        sensor_id_labels[i].config(state="disabled")
                    except Exception:
                        pass
                sensor_id_labels[0].config(state="normal")
                sensor_id_labels[0].focus_set()
        except Exception:
            pass
...
def delete_action():
    for key, entry in entries.items():
        ...
        if key in ["Current Process:", "Date and Time:"]:
            entry.config(state="readonly")

    # Clear the table and disable all sensor rows
    for i in range(20):
        try:
            sensor_id_labels[i].config(state="normal")
    ...
    # Focus Lot Number to encourage entering lot first
    try:
        ...
        try:
            ...
            if self.print_status_data:
                ...
                for row in self.print_status_data:
                    ...
                    if current_process == "Sensor Sealing":
                        try:
...
def save_action():
    try:
        ...
        for i in range(20):
            sensor_id = sensor_id_labels[i].get().strip()  # Changed from .cget("text") to .get()
            if sensor_id:
                ...
                # Collect not printed and printed lists
                if status == "Printed":
                    if sensor_id not in printed_sensor_list_local:
                        printed_sensor_list_local.append(sensor_id)
                else:
                    # Any status other than "Printed" is considered not printed
                    if sensor_id not in not_printed_sensor_list_local:
...
delete_button.place(x=465, y=115)
...
save_button.place(x=325, y=115)

# Add & Print button
# Removed Add & Print button - printing is automatic on valid sensor input
...
printer_status_label.place(x=350, y=50)
...
def update_preview(sensor_id):
    ...
    # Generate both labels (label1 now requires sensor_id)
    label1 = make_label1_image(sensor_id)
...
def auto_print_label(sensor_id, row_index):
    # Generate labels (label1 now requires sensor_id)
    label1 = make_label1_image(sensor_id)
    label2 = make_label2_image(sensor_id)
    ...
    if not detected_printer["name"]:
        ...
        # Record as Not Printed in DB
        try:
            lot_number = re.sub(r'\s+', '', entries["Lot Number:"].get())
            current_process = entries["Current Process:"].get().strip()
            operator = entries["Operator:"].get().strip()
            update_sensor_print_status(lot_number, sensor_id, current_process, 'Not Printed', operator=operator)
        except Exception:
            pass
        return False
    ...
    try:
        if method == "windows":
            ...
            # Update DB for printed sensor
            try:
                lot_number = re.sub(r'\s+', '', entries["Lot Number:"].get())
                current_process = entries["Current Process:"].get().strip()
                operator = entries["Operator:"].get().strip()
                update_sensor_print_status(lot_number, sensor_id, current_process, 'Printed', operator=operator)
                
                # Also update lot_masterlist with Sensor_Sealing status
                conn_master = sqlite3.connect(db_path_masterlist)
                cursor_master = conn_master.cursor()
                try:
                    cursor_master.execute("""
            ...
            status_labels[row_index].config(text="Printed", bg="green", fg="white")
            # Update DB for printed sensor (USB)
            try:
                lot_number = re.sub(r'\s+', '', entries["Lot Number:"].get())
                current_process = entries["Current Process:"].get().strip()
                operator = entries["Operator:"].get().strip()
        ...
        # Record Not Printed in DB on failure
        try:
            lot_number = re.sub(r'\s+', '', entries["Lot Number:"].get())
            current_process = entries["Current Process:"].get().strip()
            operator = entries["Operator:"].get().strip()
...
tk.Label(root, text="Label Preview:", font=("Arial", 10, "bold"), bg="lightblue").place(x=350, y=165)
...
# Function to process scanned sensor ID from table
def process_table_sensor_scan(row_index):
    def handler(event=None):
        sensor_id = sensor_id_labels[row_index].get().strip()
        
        if not sensor_id:
            return
        
        # Remove all whitespace
        sensor_id = re.sub(r'\s+', '', sensor_id)
        
        # Check if lot number is entered
        lot_number_raw = entries["Lot Number:"].get()
        ...
        # Validate sensor using get_allowed_sensor_ids (matching V2_Laser_Marking_and_OCR.py)
        ...
        if sensor_id not in valid_sensor_ids:
            print(f"✗ REJECTED: Sensor ID '{sensor_id}' has defects from previous processes")
            
            # Get ALL sensors in the lot to check if it belongs to this lot
            try:
                ...
                if sensor_id in all_lot_sensors:
                    ...
                    messagebox.showerror("Sensor Rejected", 
                                       ...
                                       f"Reason: Has defects from previous processes.\n"
                                       f"This sensor must be set aside.")
                else:
                    # Sensor doesn't belong to this lot
                    messagebox.showerror("Wrong Lot", 
                                       f"Sensor ID: {sensor_id}\n\n"
                ...
                messagebox.showerror("Sensor Rejected", 
                                   f"Sensor ID: {sensor_id}\n\n"
                                   f"This sensor cannot proceed to this process.")
        ...
        already_scanned = []
        for i in range(20):
            if i != row_index:  # Don't check current row
                val = sensor_id_labels[i].get().strip()
                if val:
                    already_scanned.append(val)
        ...
        # Auto print
        root.after(100, lambda: auto_print_label(sensor_id, row_index))

        # After verification, lock current row and enable next row for input
        try:
            sensor_id_labels[row_index].config(state="disabled")
        except Exception:
            pass
        if row_index < len(sensor_id_labels) - 1:
            try:
                sensor_id_labels[row_index + 1].config(state="normal")
                sensor_id_labels[row_index + 1].focus_set()
            except Exception:
                pass
        if row_index < 19:
            sensor_id_labels[row_index + 1].focus_set()
...
for row in range(20):
    ...
    # Sensor ID entry (changed from Label to Entry for barcode scanner input)
    sensor_entry = tk.Entry(root, font=("Arial", 8), bg="white", width=20, relief="ridge", justify='left')
    sensor_entry.place(x=45, y=185 + row*23)
    # Start locked; will be enabled after lot validation or when prior row is filled
    try:
        sensor_entry.config(state='disabled')
    except Exception:
        pass
    sensor_entry.bind("<Return>", process_table_sensor_scan(row))
...
# Focus first sensor ID entry in table for barcode scanner
sensor_id_labels[0].focus_set()
