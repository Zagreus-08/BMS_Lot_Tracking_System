...
class XYMoveClass(MotorClass, PortDefineClass, SystemFuncClass):
    ...
    def Xstart_with_accel(self, target_speed_hz, accel_time=0.5):
        """Start X PWM with acceleration from 0 to target speed over accel_time seconds."""
        try:
            # Stop any Y motion first
            try:
                self.Ystop()
                time.sleep(0.01)
            except Exception:
            ...
            # Acceleration ramp: start at 10% of target speed
            start_speed = max(10, int(target_speed_hz * 0.1))
            steps = 10  # Number of acceleration steps
            step_duration = accel_time / steps
            
            for i in range(steps + 1):
                if SystemFuncClass.stop_flag or self.EMGSwitch():
                    break
                # Linear acceleration ramp
                current_speed = int(start_speed + (target_speed_hz - start_speed) * (i / steps))
                pi.set_PWM_frequency(PortDefineClass.STEP1, current_speed)
                pi.set_PWM_frequency(PortDefineClass.STEP2, current_speed)
                if i == 0:
                    # Start PWM on first step
                    pi.set_PWM_dutycycle(PortDefineClass.STEP1, 50)
                    pi.set_PWM_dutycycle(PortDefineClass.STEP2, 50)
                time.sleep(step_duration)
            
            print(f"[ACCEL] X motor accelerated to {target_speed_hz} Hz over {accel_time}s")
        except Exception as e:
            print(f"Xstart_with_accel error: {e}")

    def Xstop_with_decel(self, decel_time=0.3):
        """Stop X PWM with deceleration from current speed to 0 over decel_time seconds."""
        try:
            # Get current frequency
            try:
                current_speed = pi.get_PWM_frequency(PortDefineClass.STEP1)
            except Exception:
                current_speed = 100  # Fallback if can't read
            
            # Deceleration ramp
            steps = 8  # Number of deceleration steps
            step_duration = decel_time / steps
            
            for i in range(steps, 0, -1):
                if SystemFuncClass.stop_flag or self.EMGSwitch():
                    break
                # Linear deceleration ramp
                ramp_speed = int(current_speed * (i / steps))
                ramp_speed = max(10, ramp_speed)  # Minimum 10 Hz
                pi.set_PWM_frequency(PortDefineClass.STEP1, ramp_speed)
                pi.set_PWM_frequency(PortDefineClass.STEP2, ramp_speed)
                time.sleep(step_duration)
            ...
            print(f"[DECEL] X motor decelerated over {decel_time}s")
        except Exception as e:
            print(f"Xstop_with_decel error: {e}")
    ...
    def Ystart_with_accel(self, target_speed_hz, accel_time=0.5):
        """Start Y PWM with acceleration from 0 to target speed over accel_time seconds."""
        try:
            # Stop any X motion first
            try:
                self.Xstop()
                time.sleep(0.01)
            except Exception:
            ...
            print(f"[ACCEL] Y motor accelerated to {target_speed_hz} Hz over {accel_time}s")
        except Exception as e:
            print(f"Ystart_with_accel error: {e}")

    def Ystop_with_decel(self, decel_time=0.3):
        """Stop Y PWM with deceleration from current speed to 0 over decel_time seconds."""
        try:
            # Get current frequency
            try:
                current_speed = pi.get_PWM_frequency(PortDefineClass.STEP1)
            ...
            print(f"[DECEL] Y motor decelerated over {decel_time}s")
        except Exception as e:
            print(f"Ystop_with_decel error: {e}")
...
class SimpleScanClass:
    def __init__(self, xymove=None, row_step=None, x_speed=None, y_speed=None, gui=None, x_count=None, y_count=None, 
                 threshold_mm=10.0, decel_buffer_mm=5.0, gui_ref=None):
        ...
        self.gui_ref = gui_ref  # Reference for timing updates
        
        # Store safety parameters
        self.threshold_mm = float(threshold_mm)
        self.decel_buffer_mm = float(decel_buffer_mm)
    ...
    def scan_x_by_pulses(self, direction, target_pulses, hold_after_limit=0.05):
        ...
        # Configure direction and start PWM/counters with acceleration
        try:
            if direction == "LEFT":
                ...
                self.xy.XmotorSet(0, self.x_speed)
                direction_sign = +1
            self.xy.Xstart_with_accel(self.x_speed, accel_time=0.5)
        except Exception:
            pass
        ...
        try:
            while True:
                ...
                if pulses_moved >= target_pulses:
                    ...
                    try:
                        self.xy.Xstop_with_decel(decel_time=0.3)
                    except Exception:
                        pass
                    break
    ...
    def scan_x_to_position_mm_corrected(self, target_mm, hold_after_limit=0.05, collect_data=False, current_y_mm=0.0, use_threshold=True, threshold_mm=50.0, speed_multiplier=2.0):
        """
        ...
        # Configure motor and start counting with acceleration
        try:
            if direction == "LEFT":
                self.xy.XmotorSet(1, actual_speed)
            else:
                self.xy.XmotorSet(0, actual_speed)
            self.xy.Xstart_with_accel(actual_speed, accel_time=0.5)
        except Exception:
            pass
        ...
        # Add deceleration start threshold - start decelerating earlier
        DECEL_START_BUFFER = 200.0  # Start decelerating 5mm before target
        decel_started = False
            ...
            while True:
                ...
                if use_threshold:
                    try:
                        ...
                        # Start deceleration early when approaching target
                        if not decel_started:
                            if direction == "LEFT":
                                if current_internal_mm <= (target_internal_mm + threshold_mm + DECEL_START_BUFFER):
                                    print(f"X deceleration zone: current={current_internal_mm:.2f}, target={target_internal_mm:.2f}")
                                    decel_started = True
                            else:
                                if current_internal_mm >= (target_internal_mm - threshold_mm - DECEL_START_BUFFER):
                                    print(f"X deceleration zone: current={current_internal_mm:.2f}, target={target_internal_mm:.2f}")
    ...
    def move_y_down(self, hold_after_limit=0.05):
        ...
        try:
            ...
            self.xy.Ystart_with_accel(self.y_speed, accel_time=0.5)  # Accelerate smoothly
        except Exception:
            pass
        ...
        try:
            while True:
                ...
                if self.xy.CheckYlimit_neg():
                    try:
                        self.xy.Ystop_with_decel(decel_time=0.3)
                    except Exception:
                        pass
                    return False
    ...
    def move_x_left(self, step_pulses, hold_after_limit=0.05):
        ...
        try:
            ...
            self.xy.Xstart_with_accel(self.x_speed, accel_time=0.5)  # Accelerate smoothly
        except Exception:
            pass
    ...
    def scan_y_to_position_mm_corrected(self, target_mm, hold_after_limit=0.05, collect_data=False, current_x_mm=0.0, use_threshold=True, threshold_mm=50.0, speed_multiplier=2.0):
        """
        ...
        try:
            ...
            self.xy.Ystart_with_accel(actual_speed, accel_time=0.5)
        except Exception:
            pass
        ...
        # Add deceleration start threshold - start decelerating earlier
        DECEL_START_BUFFER = 50.0  # Start decelerating 5mm before target
        decel_started = False
                ...
                if use_threshold:
                    try:
                        ...
                        if not decel_started:
                            if direction == "DOWN":
                                if current_internal_mm <= (target_internal_mm + threshold_mm + DECEL_START_BUFFER):
                                    print(f"Y deceleration zone: current={current_internal_mm:.2f}, target={target_internal_mm:.2f}")
                                    decel_started = True
                            else:
                                if current_internal_mm >= (target_internal_mm - threshold_mm - DECEL_START_BUFFER):
                                    print(f"Y deceleration zone: current={current_internal_mm:.2f}, target={target_internal_mm:.2f}")
    ...
    def scan_up_until_threshold(self, threshold_mm=50.0, current_x_mm=0.0, hold_after_limit=0.05, collect_data=True):
        """
        ...
        try:
            ...
            self.xy.Ystart_with_accel(self.y_speed, accel_time=0.5)
        except Exception:
            pass
    ...
    def simple_scan(self):
        ...
        print("Using ACCELERATION/DECELERATION for smooth motor control")
    
        if not (self.y_count and self.y_count > 0):
            print("simple_scan: y_count not provided or invalid")
            return
    
        # Use instance threshold and decel buffer settings (editable via UI)
        THRESHOLD_MM = self.threshold_mm
        DECEL_START_BUFFER = self.decel_buffer_mm
        
        # Speed multiplier for return movements (no data collection)
        RETURN_SPEED_MULTIPLIER = 3.0  # 3x faster for return movements
        ...
        print(f"=== PHASE 1 COMPLETE (100%) - Duration: {phase1_duration:.1f} seconds ({phase1_duration/60:.1f} minutes) ===")
        ...
        print(f"=== PHASE 2 COMPLETE (100%) - Duration: {phase2_duration:.1f} seconds ({phase2_duration/60:.1f} minutes) ===")
        ...
        print(f"Phase 1 Duration: {phase1_duration:.1f}s ({phase1_duration/60:.1f} min)")
        print(f"Phase 2 Duration: {phase2_duration:.1f}s ({phase2_duration/60:.1f} min)")
        print(f"Total Scan Duration: {total_scan_duration:.1f}s ({total_scan_duration/60:.1f} min)")
        print("=" * 60)
...
class DataScanClass:
    ...
    def ScanRoutine(self, row_step, x_speed, y_speed, x_count, y_count, threshold_mm=10.0, decel_buffer_mm=5.0, gui_ref=None):
        """
        ...
        Also takes threshold_mm and decel_buffer_mm for safety parameters.
        gui_ref is passed to update timing labels during scan.
        """
        ...
        try:
            ...
            simple = SimpleScanClass(self.xymove, row_step=row_step, x_speed=x_speed, y_speed=y_speed, gui=self.gui,
                                     ...
                                     threshold_mm=threshold_mm, decel_buffer_mm=decel_buffer_mm, gui_ref=gui_ref)
            simple.simple_scan()
...
class GUIClass(PortDefineClass):
    ...
    def __init__(self):
        ...
        # Threshold and Decel Buffer entries (new safety parameters)
        self.threshold_label = tk.Label(self.win, text="Threshold (mm):", font=self.labelFont, bg='#0046ad', fg='white')
        self.threshold_label.place(x=520, y=380)
        self.threshold_entry = tk.Entry(self.win, font=TkFont.Font(size=14), width=4)
        self.threshold_entry.place(x=680, y=380)
        
        self.decel_label = tk.Label(self.win, text="Decel Buffer (mm):", font=self.labelFont, bg='#0046ad', fg='white')
        self.decel_label.place(x=520, y=415)
        self.decel_entry = tk.Entry(self.win, font=TkFont.Font(size=14), width=4)
        self.decel_entry.place(x=680, y=415)
        
        # Populate threshold and decel from config or use defaults
        try:
            threshold_val = self.saved_config.get("threshold_mm", 10.0)
            self.threshold_entry.insert(0, str(int(threshold_val)))
        except Exception:
            self.threshold_entry.insert(0, "10")
        
        try:
            decel_val = self.saved_config.get("decel_buffer_mm", 5.0)
            self.decel_entry.insert(0, str(int(decel_val)))
        except Exception:
            self.decel_entry.insert(0, "5")
        
        # Scan timing display (read-only labels)
        self.timing_title = tk.Label(self.win, text="Scan Timing:", font=self.labelFont, bg='#0046ad', fg='white')
        self.timing_title.place(x=340, y=420)
        
        self.phase1_time_label = tk.Label(self.win, text="Phase 1: --:--", font=TkFont.Font(size=11), bg='#0046ad', fg='white')
        self.phase1_time_label.place(x=340, y=445)
        
        self.phase2_time_label = tk.Label(self.win, text="Phase 2: --:--", font=TkFont.Font(size=11), bg='#0046ad', fg='white')
        self.phase2_time_label.place(x=340, y=465)
        
        self.total_time_label = tk.Label(self.win, text="Total: --:--", font=TkFont.Font(size=11, weight='bold'), bg='#0046ad', fg='yellow')
        self.total_time_label.place(x=450, y=445)
    ...
    def scan_started(self):
        ...
        # Unpack returned internal units, counts, and safety parameters
        row_pulses, x_speed_hz, y_speed_hz, x_count, y_count, threshold_mm, decel_buffer_mm = valid

        def _scan_worker():
            try:
                self.reset_pulse_counters()
                # run scan routine (this will call homing internally as needed)
                # Pass threshold and decel buffer to scan routine
                self.data_scan.ScanRoutine(row_pulses, x_speed_hz, y_speed_hz, x_count, y_count, 
                                          threshold_mm, decel_buffer_mm, self)
            except Exception as e:
                print("Scan Error:", e)
                traceback.print_exc()
            finally:
    ...
    def validate_and_save_params(self):
        ...
        # Read threshold and decel buffer
        try:
            threshold_s = self.threshold_entry.get().strip()
        except Exception:
            threshold_s = "10"
        
        try:
            decel_s = self.decel_entry.get().strip()
        except Exception:
            decel_s = "5"
        ...
        try:
            threshold_mm = float(threshold_s)
            if threshold_mm < 0:
                raise ValueError
        except Exception:
            messagebox.showerror(
                "Invalid Parameter",
                "Threshold must be a positive number (mm)."
            )
            return None
        
        # Decel Buffer validation
        try:
            decel_buffer_mm = float(decel_s)
            if decel_buffer_mm < 0:
                raise ValueError
        except Exception:
            messagebox.showerror(
                "Invalid Parameter",
                "Decel Buffer must be a positive number (mm)."
            )
            return None
        ...
        cfg.update({
            ...
            "decel_buffer_mm": float(decel_buffer_mm)
        })
        ...
        # Return INTERNAL units and counts, plus threshold and decel buffer
        return (row_pulses, x_speed_hz, y_speed_hz, x_count, y_count, threshold_mm, decel_buffer_mm)
